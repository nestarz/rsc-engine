...
export const cloneElement = function (element, config, children) {
  ...
  if (null != config) {
    for (
      propName
        in (void 0 !== config.ref && (owner = void 0),
...
export const createElement = function (type, config, children) {
  ...
  if (null != config) {
    for (
      propName in (void 0 !== config.key && (key = "" + config.key), config)
    ) {
      hasOwnProperty.call(config, propName) &&
        "key" !== propName &&
        "__self" !== propName &&
...
export const startTransition = function (scope) {
...
export default { useMemo, useId, useDebugValue, useCallback, useActionState, use, startTransition, memo, lazy, forwardRef, createRef, createElement, cloneElement, cache, Suspense, StrictMode, Profiler, Fragment, Children, __SERVER_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE };
