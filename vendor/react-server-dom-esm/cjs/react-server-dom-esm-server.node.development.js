...
var ReactSharedInternalsServer = // $FlowFixMe: It's defined in the one we resolve to.
  React.__SERVER_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE || React.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
...
function writeStringChunk(destination, stringChunk) {
  ...
  if (read < stringChunk.length) {
    ...
    currentView = new Uint8Array(VIEW_SIZE);
    writtenBytes =
      textEncoder.encodeInto(stringChunk.slice(read), currentView).written;
  }
...
function preconnect(href, crossOrigin) {
  if (typeof href === "string") {
    ...
    if (request) {
      var hints = getHints(request);
      var key = "C|" + (crossOrigin == null ? "null" : crossOrigin) + "|" +
        href;
  ...
  if (isArray(objectOrArray)) {
    ...
    if (objectOrArray.$$typeof === REACT_ELEMENT_TYPE) {
      ...
      for (var _i2 = 0; _i2 < names.length; _i2++) {
        ...
        if (
          name === expandedName && typeof _value2 === "object" &&
          _value2 !== null
        ) {
...
function createRequest(
  ...
  {
    request.environmentName = environmentName === undefined
      ? "Server"
      : environmentName;
  }
          ...
          if (iterator === iterableChild) {
            var isGeneratorComponent = // $FlowIgnore[method-unbinding]
              Object.prototype.toString.call(Component) ===
                "[object GeneratorFunction]" && // $FlowIgnore[method-unbinding]
...
function renderModel(request, task, parent, key, value) {
  ...
  try {
    ...
    var wasReactNode = typeof model === "object" && model !== null &&
    ...
    if (/^on[A-Z]/.test(parentPropertyName)) {
      ...
      throw new Error(
        "Functions are not valid as a child of Client Components. This may happen if " +
          "you return " + componentName + " instead of <" + componentName +
          " /> from render. " +
...
function createResponse(bundlerConfig, formFieldPrefix) {
  var backingFormData = arguments.length > 2 && arguments[2] !== undefined
    ? arguments[2]
    : new FormData();
  var chunks = new Map();
  var response = {
...
function decodeAction(body, serverManifest) {
  ...
  body.forEach(function (value, key) {
    ...
    if (key.startsWith("$ACTION_REF_")) {
      ...
      var metaData = decodeBoundActionMetaData(
        body,
        serverManifest,
        formFieldPrefix,
      );
